...
namespace BasicMmethodExtensionClass.CacheOperationHelper.CacheHelper.ThreadHelper
{
    ...
    public class SenparcMessageQueue
    {
        ...
        /// 列队数据集合
        /// </summary>
        private static readonly Dictionary<string, SenparcMessageQueueItem> MessageQueueDictionary = new Dictionary<string, SenparcMessageQueueItem>(StringComparer.OrdinalIgnoreCase);

        /// <summary>
        /// 同步执行锁
        /// </summary>
        private static readonly object MessageQueueSyncLock = new object();
        /// <summary>
        /// 立即同步所有缓存执行锁（给OperateQueue()使用）
        /// </summary>
        private static readonly object FlushCacheLock = new object();
