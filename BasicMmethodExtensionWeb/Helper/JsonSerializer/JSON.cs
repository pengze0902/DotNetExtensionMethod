...
using System.Text;
using BasicMmethodExtensionWeb.Helper.DataTableHelper;
using Helper.fastJSON;

namespace BasicMmethodExtensionWeb.Helper.JsonSerializer
{
	public class JSON
	{
		...
		{
			var ht = (Dictionary<string, object>)JsonParser.JsonDecode(json);
		    return ht;
	    ...
	    readonly SafeDictionary<string, PropertyInfo> _propertycache = new SafeDictionary<string, PropertyInfo>();
        ...
        {
            StringBuilder _output2 = new StringBuilder();
            _output2.Append('\"');
            ...
            foreach (char c in s)
            {
                switch (c)
                {
                    case '\t': _output2.Append("\\t"); break;
                    case '\r': _output2.Append("\\r"); break;
                    case '\n': _output2.Append("\\n"); break;
                    case '"':
                    case '\\': _output2.Append("\\"); _output2.Append(c); break;
                    default:
                        {
                            if (c >= ' ' && c < 128)
                                _output2.Append(c);
                            ...
                            {
                                _output2.Append("\\u");
                                _output2.Append(((int)c).ToString("X4"));
                            }
                        }
                        break;
                }
            ...
            return _output2.Append('\"').ToString();
        }
	}
}
