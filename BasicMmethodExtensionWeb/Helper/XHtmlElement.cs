...
namespace BasicMmethodExtensionWeb.Helper
{
    ...
    public class XHtmlElement
    {
        ...
        /// <returns></returns>
        public List<HtmlInfo> Descendants(string elementName)
        {
            if (string.IsNullOrEmpty(_html))
            {
                throw new ArgumentNullException(_html);
            }
        ...
        public List<HtmlInfo> RootDescendants(string html = null)
        {
            ...
            var eleList = new List<string>();
            var reval = new List<HtmlInfo>();
            GetElementsStringList(html, ref eleList);
            foreach (var r in eleList)
            {
                var data = new HtmlInfo
                {
                    OldFullHtml = r,
                    SameLeveHtml = html,
                    TagName =Regex.Match(r, @"(?<=\s{1}|\<)[a-z,A-Z]+(?=\>|\s)",RegexOptions.IgnoreCase).Value,
                    InnerHtml =Regex.Match(r, @"(?<=\>).+(?=<)", RegexOptions.Singleline).Value
                };
        ...
        private List<HtmlInfo> GetDescendantsSource(IEnumerable<HtmlInfo> allList, string elementName)
        {
        ...
        private void GetElementsStringList(string html, ref List<string> eleList)
        {
            ...
            {
                TagName =
                    System.Text.RegularExpressions.Regex.Match(html, @"(?<=\<\s{0,5}|\<)([a-z,A-Z]+|h\d{1})(?=\>|\s)",
                        RegexOptions.IgnoreCase).Value
            };
            var currentTagBeginReg = @"<\s{0,10}" + info.TagName + @".*?>";//获取当前标签元素开始标签正则
            var currentTagEndReg = @"\<\/" + info.TagName + @"\>";//获取当前标签元素收尾标签正则
    ...
    public static class XHtmlElementExtendsion
    {
        ...
        public static List<HtmlInfo> ParentDescendant(this  IEnumerable<HtmlInfo> htmlInfoList, string fullHtml)
        {
            ...
            var xhe = new XHtmlElement(parentHtml);
